...
void Path::addSegment(const Node3D& node) {
  ...
  vertex.pose.position.x = (0.1*node.getX() * Constants::cellSize)-15;
  vertex.pose.position.y = (0.1*node.getY() * Constants::cellSize)-15;
...
void Path::addNode(const Node3D& node, int i) {
  ...
  pathNode.header.frame_id = "camera_init";
...
void Path::addVehicle(const Node3D& node, int i) {
  ...
  pathVehicle.header.frame_id = "camera_init";
